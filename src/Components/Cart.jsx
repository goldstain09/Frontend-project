...
import {
  addToCartStartAc_Fn,
  decreasingQuantityStartAc_Fn,
  increasingQuantityStartAc_Fn,
  removeProductStartAc_Fn,
} from "../Redux/Actions";
        ...
        {cart.length > 0 ? (
          cart.map((item, index) => (
            <div className="row" key={index}>
              ...
              <div className="col-12 col-sm-12 col-md-12 col-lg-7 col-xl-7 title_Div">
                ...
                <h2>
                  <span>${(item.price + 25).toFixed(2)}</span> ${item.price}
                </h2>
              </div>
          ...
          <>
            <div className="text-center">
              <h1
                className="h1 text-center mb-5 mt-5"
                style={{
                  color: "#97ddf4",
                  fontFamily: "'Raleway', sans-serif",
                }}
              >
                Cart is Empty <i class="bi bi-emoji-frown"></i>
              </h1>
              <Link
                to={"/Frontend-project"}
                className="w-100 text-center text-decoration-none"
              >
                ADD Items <i class="bi bi-cursor"></i>
              </Link>
            </div>
          </>
        )}
        ...
        {total > 0 && (
          <div className="row d-flex justify-content-center">
            <div className="col-12 total">
              <h3 align="center" className="h3">
                Delivery Charges:${+10}
              </h3>
              <h3 align="center" className="h3">
                Products Total:${total.toFixed(2)}
              </h3>
              <h1 align="center" className="h1">
                Grand Total:${(total + 10).toFixed(2)}
              </h1>
              <button
                onClick={() => {
